...
- `tests/hermes_cli/test_status.py` 全文 **10 个用例**,主题分别是:
  ...
  OAuth 授权存储的显示、以及五支**抗崩溃**测试(import 失败 / 函数抛异常 /
  返回 None 都不能让 `show_status` 崩)。**没有任何一支断言平台表那一行的内容。**
...
一类),**结果为零**。这与本轮 ◇ 条目的分布吻合:配置键的文档覆盖率
(87.7%,见 `notes/r8a-90`)完全依赖人工维护,没有任何自动关卡把
`DEFAULT_CONFIG` 与文档对表 —— 而环境变量那一侧覆盖率 100%,靠的也不是自动化,
是 `OPTIONAL_ENV_VARS` 每条自带 `description` 字段这一**结构性耦合**
(定义与说明写在同一个字面量里,想漏都难)。
