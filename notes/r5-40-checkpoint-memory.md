...
5. **一致项(抽查确认)**:触发命令清单(docs `checkpoints-and-rollback.md:32` ↔ `agent/tool_dispatch_helpers.py:75-89`)、存储布局(docs `:218-228` ↔ 模块头 `:16-25`)、冻结快照语义(memory.md:57 ↔ `tools/memory_tool.py:11-14`)、"外部 provider additive、一次一个"(memory-providers.md:9 ↔ `memory_manager.py:6-8` + `agent/system_prompt.py:515-531`)、write_approval 决策矩阵(memory.md:250-253 ↔ `tools/write_approval.py:264-272`)均相符。
