...
**共 17 条。编号跳过 H-8**:该编号在成稿过程中空出,**主线没有留下它当时指什么的记录**,
定稿复盘时全仓 grep `H-8` 零命中。**不补编号、也不编一个理由**——
空号本身是无害的,而给它安一个想当然的来历会污染后续轮的引用。
(这条小事值得写下来,因为它和本卷反复出现的判据是同一条:
**不知道就说不知道,不要用一个听起来合理的解释填坑。**)

> **R8-fix 修正(review-1 建议-18 / M-22)**:上一句原写"共 **16** 条",而紧随其后的移交表
> 实为 **17** 行(H-1…H-18,跳过 H-8),同轮报告 `reports/round-8a-*.md:676` 写的 17 才是对的。
> **错在证据层而不是摘要层,这比反过来更麻烦**:CLAUDE.md 的分工是"底稿是证据层",
> 下一轮若按制度回到**底稿**核移交清单,拿到 16 会误以为漏传或多传了一条。
