...
| `H-R11F-M-e` | §4 那 59 条「转内容轮」只经过了**锚点子树**这一层复核,没有逐条读它的现象描述;而 `H-R10G-b` 正是靠读现象描述才发现它横跨两棵子树(见 §4.1)。锚点:`data/r11e/handover-candidates.tsv:1`:`case_id	source	verdict	one_line	anchor` | 下一个内容轮:**不得直接引用 §4 的结论**,须逐条读 `one_line` 再判 |
| `H-R11F-M-f` | **制度候选项(交装订轮)**:「章正文冻结」这类边界与**声明式派生值**天然会撞 —— 本轮 `chapters/r1` 的三个 `<!-- derived: -->` 值因台账变更而失效,冻结边界与派生数字关卡直接冲突,当场需要人来裁。裁定已由卡主给出并应写进制度:**派生值是台账的投影,不属于「作者撰写的内容」,源变则必须同步,且同步不计作对该章的改动;声明之外一个字都不许动。** 锚点:`chapters/r1-what-is-hermes-agent.md:128`:`<!-- derived: ledger.inventoried.files ledger.inventoried.lines ledger.total.files ledger.processed.files -->` | R12 装订轮:把这条写进 `CLAUDE.md` 的「历史产出的改法」一节,与「行号漂移」「锚点寻址修正」两条例外并列为第五类 |
