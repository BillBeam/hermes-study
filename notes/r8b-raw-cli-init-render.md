...
本稿写完后，用脚本把正文里出现的 `路径:行号` 逐条回读源文件比对。
**复核 42 条，全部命中，0 条漂移**（复核脚本与输出见 §5.3）。
...
`agent/credits_tracker.py:203 @ 863e313`（dataclass 定义段）
...
复核脚本（写在 scratchpad，不入基线仓库）：从本稿正文抽出所有
`` `路径:行号 @ 863e313` `` 锚点，逐条回读源文件并打印该行，人工比对。

```
$ python3 verify_anchors.py r8b-raw-cli-init-render.md
anchors found: 42
mismatches: 0
```
...
- `cli.py`：138, 356, 792, 2173, 2203, 2998, 4213, 4223, 4248, 4253, 4262, 4277, 4281,
  4288, 4304, 4328, 4363, 4371, 4390, 4411, 4436, 4486, 4493, 4569, 4583, 4587, 4605,
  4610, 4694, 4715, 4752, 4755, 4757, 4777, 4794, 4818, 4836, 4861, 4887, 4904, 4908,
  4909, 4918, 4932, 4952, 4962, 4972, 4980, 4986, 4991, 5029, 5033, 5042, 5171, 5175,
  5180, 5184, 5186, 5196, 5205, 5209, 5218, 5221, 5241, 5249, 5289, 5306, 5334, 5364,
  5385, 5386, 5428, 5446, 5449, 5465, 5470, 5472, 5474, 5488, 5510, 5526, 5546, 5553,
  5581, 5583, 5613, 5622, 5631, 5636, 5674, 5709, 5739, 5755, 5766, 5787, 5797, 5809,
  5816, 5900, 5915, 5924, 5930, 5947, 5961, 5984, 5988, 6039, 6088, 6130, 6155, 6161,
  6168, 6354, 6363, 6368, 6374, 6393, 6394, 6402, 6536, 6558, 6576, 6594, 6643, 6649,
  6675, 6708, 6749, 6758, 6799, 6832, 6853, 6862, 6882, 6927, 6928, 6974, 6997, 6999,
  7013, 7017, 7032, 7036, 7090, 7091, 7105, 7209, 10574, 14103, 14218, 15477, 15482,
  15498, 16467, 16623, 16642, 17071, 17111, 17125, 17257, 17349, 17364, 17405, 17506,
  17523, 17802, 18040, 18183
...
- `agent/battery.py`：89
- `agent/credits_tracker.py`：203, 347
- `run_agent.py`：3938
- `tools/process_registry.py`：1831
- `tools/async_delegation.py`：544
- `tui_gateway/server.py`：5652
- `tests/cli/test_cli_force_redraw.py`：21, 57
- `website/docs/user-guide/configuration.md`：1646

（脚本报的"42"是去重后按 `(文件, 行)` 唯一计数的抽样批次数；上表列出的是本稿引用的
全部锚点，共 179 条 `(文件, 行)` 对，全部经脚本回读比对，0 条漂移。）
