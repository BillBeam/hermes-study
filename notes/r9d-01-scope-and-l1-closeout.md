...
**所以本轮对历史积压的真实贡献是 40 → 38(清掉 2 个)**,不是 40 → 17。
清掉的是 `agent/jiter_preload.py`(F 片讲启动期惰性 import 时真的引用)与一个 `hermes_cli/subcommands/`
下的文件(移交取证组 D 讲凭据落盘链路时引用),均属**真覆盖**。
按原属轮重新归并:**R2 3 / R4 4 / R6 2 / R7B 12 / R8B 17 = 38**。

*(收工复测的数与开工中途那次不同:朴素读数 18→17、去污染读数 39→38,
因为期间又有底稿落地。**朴素读数会随语料增长持续漂移,去污染读数才是可比的那一个。**)*
