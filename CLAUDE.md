...
- **历史产出的改法(R8-fix)**:
  ...
  **自引锚点的 commit 钉子(R11D 立)**:基线锚点写作 `路径:行号 @ 863e313`,
  ...
  R11D 只改了 `chapters/r1` 的几个数,当场打断 7 处;全语料**自引锚点 615 处,
  其中 101 处指向 `chapters/`**(`data/r11d/probes/self_citation_census.py`),
  R12 重排全部 21 章时会一起断。**写法**:`chapters/r1-….md:103 @ 25c612f`,
  ...
  **注意 79 处历史锚点把自引路径挂在基线 sha `863e313` 上**(`scripts/` 50 / `chapters/` 9 /
  `notes/` 7 / `data/` 7 / `reports/` 5 / `reviews/` 1):那是个类别错误(sha 指的是另一个仓库),
  当前无害(自动忽略),但读者会被误导,见 R11D 遗留清单。
