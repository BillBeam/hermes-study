...
| 口径 | 全路径零命中 | 裸文件名零命中 |
|---|---|---|
| 朴素(含本轮那张积压清单表) | 18 文件 / 1,085 行 | 0 文件 / 0 行 |
| **剔除该清单表(以此为准)** | **39 文件 / 7,772 行** | **10 文件 / 2,781 行** |
...
**所以本轮对历史积压的真实贡献是 40 → 39,只清掉 1 个**
(`agent/jiter_preload.py`,F 片讲启动期惰性 import 时真的引用了它)。
真实积压按原属轮归并:**R2 3 / R4 4 / R6 2 / R7B 12 / R8B 18 = 39 文件 / 7,772 行**,
逐个点名见 `notes/r9d-01-scope-and-l1-closeout.md` §3.3 与 §3.4。

**归属(结清 H-R9C-e 的"需指定补齐轮次")**:全部 39 个归 **R11B**。建议按重量分两片——
R7B 那 12 个占积压行数的 63.8%(4,960 行,`yuanbao_proto.py` 一个就 1,418 行)单独排一片;
R8B 那 18 个平均 60 行、多为薄壳,一节讲完即可;R2/R4/R6 那 9 个零散并入。
...
| **H-R9C-a** | **立 ■,范围由 1 处扩到 2 处**;env 分支判为合乎设计、非缺陷 |
| **H-R9C-b** | 核心问号判**阴性**(落盘 `.env` 在禁读清单里) |
...
- **限定**:**39 个 L1 文件(7,772 行)在全部产出语料里没有任何一条可溯源断言**(§3.4)。
  按本项目的证据格式,这意味着它们的 `status` 列**高于实际交付**。
  **"L1 全部 deep-read"在台账意义上为真,在"每个文件都被明确交代"这个最终目的意义上尚有 39 个缺口。**

**主线判断:R12 可以启动,不必等 R11B。** 理由:R12 的任务是**装订与全局重构**已有的 17 章成品章,
而那 39 个缺口文件**没有一个是任何一章的主题**(逐个核过:18 个是 `hermes_cli/subcommands/` 薄壳、
...
1. **L1 已收口,R12 前置按字面满足**(§8),但 R12 正文必须带上那 39 个点名缺口的限定。
2. **R11B 的两笔欠账建议合并做**:六章锚点排版(H-R8D-g)与 39 个点名缺口(H-R9C-e)都是
