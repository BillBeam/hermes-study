...
8. **主线的一次边界违反(已修复)**:本轮边界明写"不把任何会话、模型标识、背景信息写入仓库产物",
   而我前 6 个 commit 的尾注同时写进了模型标识与会话链接。发现后用 `filter-branch` 重写这 6 条
   提交信息并 force-with-lease 推送;**已复核 `git diff` 为空**——树内容完全一致,只改了提交信息。
   *留痕的理由:这条边界不是产物内容的规矩,是产物元数据的规矩,而元数据不在任何一个关卡的覆盖面内
   ——四关全绿的同时它一直是破的。*
9. **本轮未做的事**:未跑真实模型/计费/云端点,未配置任何凭据;`chapters/` 六章的 UNCHECKED 欠账未动。
