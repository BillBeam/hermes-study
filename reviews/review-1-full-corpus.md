...
**加重一层——仓库自己的文档把这个区别说得很清楚,是这条定案把它们并了。** 评审位去核文档侧:
...
同页 `:240` 与 `:244` 分两行各自注释("Persist /workspace and /root bind-mount dirs" vs
"Reuse container across Hermes restarts"),`:276` 更直说 `docker_persist_across_processes: false`
才是"Every `cleanup()` does `stop` + `rm -f`"。**"distinct from" 是文档原话。**

**于是 r4 §3.7 那段"同一页自相矛盾"的定性也要重新看**:`tools.md:88` 与 `:90` 讲的是**两个不同的开关**,
`:90` 对它自己那个开关的描述是准确的。真实缺口是 `tools.md:88` **从不提** `docker_persist_across_processes`,
而它链接过去的 `configuration.md` 把这件事写对了——**这是"该页信息不全 + 未指向正确的键",不是"自打脸"**。
...
| **M-19** | **阻断** | `notes/r8b-90-*.md:19,26,57` | H-7 负结论错:`hermes_cli/auth.py:7270` 就是第三个"读原始配置后整文件落盘"的调用方,坏 YAML 下 `read_raw_config()` 返 `{}` → 落盘只剩 `model:`,`approvals.deny` 静默消失(附录 A-1) | **重开 H-7**;并复查 `require_readable_config_before_write` 全部调用点 |
| **M-20** | **阻断** | `notes/r8b-90-*.md:260-262` | H-17 收窄用了全称否定"全程没有用空 dict 覆盖",而 `env_loader.py:669` 就是无守卫整体赋值(加锁孪生 `:236` 有 `if values:`)(附录 A-2) | 撤销收窄或改述 |
| M-21 | 建议 | `reports/round-6-*.md:30` | `118+35+35+31+27=246≠243`;R3 残余实为 32(台账实测),和与 8287 都对,只有加数没跟着减 | `R3(35)` → `R3(32)` |
| M-22 | 建议 | `notes/r8a-90-*.md:2097` | 底稿写"共 16 条",其表实为 17 行,报告的 17 才对——错在证据层 | 底稿改 17 |
| M-23 | 建议 | `reports/round-8b-*.md:220` vs `:350` | H-14 在 §5 记"结清",同报告 130 行后记"未逐处落实"并派生 H-R8B-b 移交 R11 | 改为"部分结案 + 派生移交" |
| M-24 | 建议 | `reports/round-8b-*.md:308` vs `notes/r8b-02-*.md:4` | venv 报 87、同轮底稿写 89,组成描述逐字相同,报告未注明口径 | 注明 `*.dist-info` 计数口径 |
| M-25 | 建议 | R7/R7B/R7C/R8A/R8B 五份报告(全文) | 全局进度报数从 R7 起中断;台账实测仍有 **8,122** 文件为 `R1-inventoried`,而分层快照几乎不变、读不出剩余量 | 恢复"`R1-inventoried` 剩余"为每轮必报项 |
| M-26 | 建议 | `notes/r8b-90-*.md:407` | H-R8B-c 的"锚点"列无任何文件路径;所指 `r8a-90` §3.6 的 8 个位置全裸文件名,`dump.py`/`setup.py` 两个无法解析 | 补锚点;参照同表 H-R8B-a 的写法 |
| M-27 | 建议 | `notes/r8b-90-*.md:4` | 写"6 条"却列了 7 个编号,结论表也是 7 行 | 6 → 7 |
...
| `notes/r7c-90` + `r8a-90` + `r8b-90`(3,018 行,**补充轮**) | 38 | 30 | 8 | 5 |
| 全部 12 份报告的格式与报数(**补充轮**) | 50 | 41 | 9 | 4 |
| **合计** | **150** | **116** | **37** | **17** |

**补充轮的两行(见附录 A)覆盖的正是本报告初稿申报为"未覆盖"的两个范围。**
所以 §6.3 第 1 条与第 7 条的相应表述已按补审结果修订——**定案卷 10 份现已全部审过**。

**"标出待查 20 条、只立案 8 条"这个落差是有意的,请仲裁注意其含义**:
本报告的立案门槛是"评审位自己到基线取到证据",凡未亲核的一律不写进正文。
立案的 17 条 = 阻断-4/5/6 + 建议-12/13/14/15/16(建议-15 合并了 5 处锚点漂移)
+ 附录 A 的 A-1…A-9(2 条阻断 + 7 条建议)。
**其余 20 条未立案 ≠ 已澄清,而是本轮未复核**,应与 §6.3 的未覆盖项同等对待。
...
1. **`notes/` 底稿的内容正确性大部分未审**。全部 117,351 行里,本轮**只**深入了 7 份定案卷
   ...
   **(修订)`notes/r7c-90`、`notes/r8a-90`、`notes/r8b-90` 三份在补充轮已补审**
   (38 条逐条核实,立案 5 条,见附录 A);初稿此处的"完全未审"已不成立。
   仍未读的是 `r7-raw-*`、`r7c-raw-*`、`r8a-raw-*`、`r8b-raw-*` 共 40 余份分段底稿。其余底稿只被当作**核对参照**用(判断某个数是"章节笔误"还是"底稿传下来的",见建议-6)。
...
7. **原定规模被压缩,且对抗复验环节被整段砍掉**。评审位原计划分 19 路对全部章节、报告、
   ...
   **(修订)** 初稿此处曾申报"另两份范围尚未产出结果、一律未纳入";**它们随后完成并已补审**,
   结果见附录 A。仍然成立的是那句原则:**报告只写评审位已亲自复核的部分。**
...
## 附录 A · 补充轮(两份原申报"未覆盖"的范围,已补审)

§6.3 第 7 条曾申报:`notes/r7c-90` / `notes/r8a-90` / `notes/r8b-90` 三份定案卷与 12 份报告的
格式与报数复算"在本报告定稿时尚未产出结果,故一律未纳入"。**这两个范围随后完成了**,
本附录是补审结果。**下面每一条都经评审位亲自到基线复核**,未复核的一律不写。

补审规模:定案卷三份 38 条逐条核实(30 条成立)、12 份报告 50 项核实(41 项成立)。
新增 **2 条阻断、6 条建议**,已一并加入 §5 移交清单(编号 M-19 起)。

---

### A-1【阻断-7】R8B 的 H-7"负结论"不成立:第三个"读原始配置后整文件落盘"的调用方存在,且它会在坏 YAML 下抹掉 `approvals.deny`

**被评审位置**:`notes/r8b-90-handover-rulings.md:19,26,57@38b65bb`
...
**为什么不成立**:存在这样的第三个调用方,而且它不是迁移流水线——
`hermes_cli/auth.py` 的 `_update_config_for_provider`。评审位追了 `:7293 → :7329` 的整条路径:

`hermes_cli/auth.py:7293-7295 @ 863e313`
...
那道守卫按 R8A H-7 的原话只查**可读**、不查**可解析**;而 `read_raw_config()` 在解析失败时
**返回空字典**:

`hermes_cli/config.py:2961-2963 @ 863e313`
...
从 `:7295` 到 `:7329` 之间**没有任何解析检查、也没有空判**,终点是整文件替换:

`hermes_cli/auth.py:7327-7329 @ 863e313`
...
落盘文件就**只剩 `model:` 一段**,其余配置——**包括 `approvals.deny`**——静默消失。

**为什么这条最重**,有三层:

1. **它是一条"负结论"。** 正结论错了会被下一个读者撞见;**负结论错了会关闭调查**。
   R8B 写的是"H-7 关闭",按 CLAUDE.md 的移交制度,后续轮次不会再回来看这里。
2. **它恰好击穿 r8a 引以为荣的那条安全叙事。** `chapters/r8a-configuration-surface.md:568-603@38b65bb`
   用整节讲"解析失败退回上一次好的,而不是退回默认值",理由原文是
   "配置里有 `approvals.deny`……退回默认值等于**用户存了个错别字,防线就自己拆了**"。
   那一节讲的是 `load_config`;**而这条路走的是 `read_raw_config`,它返回 `{}`**——
   同一个不变式,在另一条读取路径上根本不成立。这正是 r8a/r8b 反复讲的
   "同一语义多份实现、其中一份没跟上"形状,**出现在了安全不变式上**。
3. **它是 §2 那条元判断的最硬证据**:代码侧有脚本兜着所以稳,而这条负结论的成立与否
   **完全取决于"全仓调用点有没有数全"**——那是一次 grep 的完备性,没有任何机制校验。

**建议改法**:重开 H-7,把 `hermes_cli/auth.py:7270`(`_update_config_for_provider`)记为
第三个调用方并给出上面的失效链;同时复查 `require_readable_config_before_write` 的全部调用点
是否都只保证"可读"(见 A-6)。

---

### A-2【阻断-8】R8B 对 H-17 的"收窄"用了一个全称否定,而基线里就有反例

**被评审位置**:`notes/r8b-90-handover-rulings.md:260-262@38b65bb`

**被质疑原文**(逐字):

> **但"缓存被空 dict 覆盖"这个说法不成立,应予收窄。** `_APPLIED_HOMES` 是 `set`,
> `:251` 是 `.clear()` 而非赋值,**全程没有"用空 dict 覆盖"的写法**;

**为什么不成立**:收窄只核了 `_APPLIED_HOMES` 一个容器,而 R8A H-17 说的是这条路径写的那批全局**字典**。
无锁那条路径上就有一次整体赋值:
...
`values` 只收 `name in os.environ` 的项,**完全可以是空 dict**(provenance 有项但值都不在环境里)。
而**加锁的孪生写法恰恰有守卫**:

`hermes_cli/env_loader.py:236-237 @ 863e313`
...
**这道 `if values:` 的有无,正是该节自己确认的那个"锁不对称"的另一面。**
所以"全程没有'用空 dict 覆盖'的写法"这个**全称判断**与基线矛盾,收窄失去了它声明的依据。

**为什么是阻断**:收窄一条移交项等于缩小后续轮次的排查面。这里收窄的理由是一个**可被一行代码推翻**
的全称否定,而被收掉的恰好是"无锁路径会写空字典"——**H-17 原本要查的就是这件事**。

**建议改法**:撤销该收窄,或改述为"`_APPLIED_HOMES` 是 `set` 且用 `.clear()`,这一处不涉及空 dict 覆盖;
但 `_SECRET_SOURCE_VALUES_BY_HOME` 在无锁路径 `:669` 存在无守卫的整体赋值,加锁孪生路径 `:236` 有 `if values:` 守卫,
H-17 的原判在此成立"。

---

### A-3【建议-17】`round-6` 报告的累计加数与它自己给的和差 3

**被评审位置**:`reports/round-6-memory-provider-ecosystem.md:30@38b65bb`

**被质疑原文**(逐字):

> 累计已学:R2(118)+ R3(35)+ R4(35)+ R5(31)+ R6(27)= **243 文件**,R1-inventoried 降至 8287。

`118+35+35+31+27 = 246`,不是 243。**错的是 `R3(35)` 这个加数**——同一份报告 `:37-40` 自己写了
三个 mcp_oauth 文件"从 `R3-structure` 升为 `R6-deep-read`,**R3-structure 归零**",
这三个已计入 `R6(27)`。评审位复算台账 `status` 列:
...
R3 残余确为 **32**;按 `R3(32)` 算:`118+32+35+31+27 = 243` ✓,与 `8530−243=8287` 也自洽。
**所以和与 8287 都是对的,只有 R3 那个加数没跟着减。**
(`R2(118)` 亦复核无误:`R2-structure 72 + R2-deep-read 46 = 118`。)

**值得记的是它有正例对照**:`reports/round-5-*.md:30@38b65bb` 做了同一件事——把 R4 从 39 减为 35,
因为 4 个文件被促升。**同一个动作,R5 记得减,R6 忘了。**

---

### A-4【建议-18】底稿说"共 16 条",它自己的表是 17 行,报告写的 17 才是对的

**被评审位置**:`notes/r8a-90-doc-conflict-rulings.md:2097@38b65bb` vs
`reports/round-8a-configuration-surface.md:676@38b65bb`

底稿:

> **共 16 条。编号跳过 H-8**:该编号在成稿过程中空出,**主线没有留下它当时指什么的记录**,

报告:

> **本轮移交的 17 条全部按新格式给出锚点文件(带行号)与一句话现象**

评审位数了紧随其后的移交表(`notes/r8a-90:2100-2125` 中 `^\| H-` 行):**17 行**。
**所以报告的 17 是对的、底稿的 16 是错的。**

**为什么值得报**:CLAUDE.md 的分工是"底稿是证据层"。下一轮若按制度去**底稿**核移交清单,
拿到 16 会误以为漏传或多传了一条。**当证据层与摘要层报数不一致时,错的偏偏是证据层,
这比反过来更麻烦。**

(应当肯定的核实结果:H-8 空号在 `notes/r8a-90:2097-2099` 有明确交代——"不补编号、也不编一个理由",
这是正确处置;H-1…H-18 在该表里全部带了锚点文件 + 行号 + 一句话现象。)

---

### A-5【建议-19】`round-8b` 报告说 H-14"结清",同一份报告 130 行后说它"未逐处落实"并派生了一条 R11 移交

**被评审位置**:`reports/round-8b-cli-trunk-and-interaction.md:220@38b65bb` vs `:350@38b65bb`
...
**既然派生出一条新的 R11 移交项,就不是"结清"而是"部分结案 + 派生移交"。**
这正是 CLAUDE.md 的移交制度想防的事:只看 §5 结算表的下一轮会认为 H-14 已封盘,不会去找 H-R8B-b。

**同一份报告里作者是有这个区分的**——H-13 写的是"拆开后整体移交 R8C"、H-17 写的是"确认并收窄",
措辞都准确。**只有 H-14 落在了"结清"那一栏。**

---

### A-6【建议-20】venv 包数报了 87,同轮底稿写 89,而这条报数制度是 R8A 自己立的

**被评审位置**:`reports/round-8b-cli-trunk-and-interaction.md:308@38b65bb` vs
`notes/r8b-02-h1-h2-config-debt.md:4@38b65bb`

报告:`**venv 包数:87**(`[dev]` extra + `aiohttp 3.14.1` + `brotlicffi 1.2.0.1`)`
底稿:`实跑环境:/home/user/hermes-venv(`pip list` **89 个包** = `[dev]` extra + `aiohttp 3.14.1` + `brotlicffi 1.2.0.1`)`

**括号里的组成描述逐字相同,数字差 2。** 两者很可能都对(87 是 `*.dist-info` 计数、
89 是 `pip list` 计数,后者多出 pip/setuptools 之类),**但报告没写自己用的是哪种口径**,
于是 87 与 89 无法互证。

**这是 R8A 亲手立的制度的第一次执行。** CLAUDE.md 立它时的原话是
"查证方法与'基线是否干净'同理——**直接断言,不要间接推断**:去看 `site-packages/*.dist-info` 的时间戳,而不是猜"。
一个不注明口径、且与本轮底稿冲突的数,恰好落回了制度想消灭的那个状态。

**建议改法**:报数时写明口径(`ls -d .../*.dist-info | wc -l` = 87),并把底稿的 `pip list` 数一并留着作对照。

---

### A-7【建议-21】全局进度报数从 R7 起中断,实际还剩 8,122 个文件没开工

**被评审位置**:`reports/round-7-*.md` / `round-7b` / `round-7c` / `round-8a` / `round-8b`(全文)

R2–R6 每轮都报全局进度:`round-2:34`"R1-inventoried | 8412"、`round-3:45`"降至 8377"、
`round-4:31`"降至 8338"、`round-5:30`"降至 8311"、`round-6:30`"降至 8287"。
**从 R7 起这条线索断了**——R7/R7B/R7C/R8A/R8B 全文 grep `累计已学` 与 `R1-inventoried` **零命中**,
只报本轮文件数与五层快照。
...
**仍有 8,122 个文件的 status 是 `R1-inventoried`**,即 8,530 − 8,122 = 408 个文件被真正处理过。

**为什么这条对项目本身最有用**:五层快照(L1/L2/L3/L4/LT)每轮几乎不变——L3/L4/LT 连续五轮一字未动——
所以**读者从报告里读不出"还剩多少没开工"**。而 CLAUDE.md 的最终目的第 3 条是
"全仓每一个源文件都被明确交代,没有黑洞",它的**唯一可观测指标恰恰是 status 列**,不是分层列。
**报了不变的那个数、停报了会变的那个数。**

**建议改法**:把"`R1-inventoried` 剩余文件数 / 行数"恢复为每轮必报项,与分层快照并列。

---

### A-8【建议-22】一条移交项的"锚点"列里没有任何文件路径,而它指向的表里 8 个位置有 2 个无法解析

**被评审位置**:`notes/r8b-90-handover-rulings.md:407@38b65bb`

**被质疑原文**(逐字):

> | H-R8B-c | R8C | 平台就绪判定的 8 份实现(表见 R8A `notes/r8a-90` §3.6) | 逐份对齐的清点未做,落点与 web 面重叠更大 |

"锚点"列里**只有一句描述加一个指向另一份文档表格的指针,没有任何文件路径**——
正是 CLAUDE.md 明令禁止的形态,而那条规矩的立规理由原文就是
"R7B 更有一条只在'下一轮建议'里出现过标题、从未取证,却被当成已取证结论传了下去"。

**加重一层**:被指向的 `notes/r8a-90` §3.6 首行给出的 8 个位置**全是裸文件名、无行号**,
其中两个在基线里无法解析——`dump.py` 在仓库根不存在(只有 `hermes_cli/dump.py`),
`setup.py` 同时存在于根目录与 `hermes_cli/` 两处。**所以 R8C 拿到这条时仍需重新定位,
移交项的目的落空。**

**同表有达标写法可对照**:`H-R8B-a` 给了 `config_defaults.py:2129` + `config.py:4660`——
评审位核过,两处逐字命中,且顶层 `personalities` 确实零读取点。**作者知道怎么写,只是这一条没写。**

---

### A-9【建议-23】"本轮认领 6 条",紧跟着列了 7 个编号

**被评审位置**:`notes/r8b-90-handover-rulings.md:4@38b65bb`

> 本轮认领 R8A 移交里标注 **R8B** 的 6 条:H-1 / H-2 / H-7 / H-13 / H-14 / H-16 / H-17

列了 7 个,写的是 6;`:7-15` 的结论表也是 **7** 行。按 R8A 移交表,标注含 R8B 的确实是这 7 条。
**这不是基线冲突而是文件自洽问题,但它落在开篇的"本轮范围"声明上——下一轮据此对账会拿到一个错的分母。**

---

## 7. 给仲裁的五句话

1. **八条阻断没有一条是"机制理解错"。** 四条是数字/数量级或计数写错(阻断-2/3/5 与附录 A-2 的全称否定),
   四条是"代码读对了、指错了对象"(阻断-1 指错层、阻断-4 指错文档小节、阻断-6 指错配置键、
   附录 A-1 漏数了一个调用点)。§6.2 亲核的断言里,凡涉及"这个机制为什么这样设计"的**全部成立**。
...
4. **附录 A 那两条阻断请优先看,尤其 A-1。** 它推翻的是一条**负结论**——R8B 写"H-7 关闭",
   按移交制度后续轮次不会再回来看;而被漏掉的那个调用方(`hermes_cli/auth.py:7270`)
   会在坏 YAML 下把用户的 `approvals.deny` 静默抹掉。**它同时说明 r8a 引以为荣的
   "解析失败退回上一次好的"这条安全不变式,在 `read_raw_config` 那条读取路径上根本不成立**——
   同一语义两份实现、其中一份没跟上,这次落在了安全不变式上。
5. **本报告自身的覆盖仍不均衡,请按 §6.3 打折使用。** 11 章全读、机器校验全量、
   10 份定案卷全部审过(补充轮补齐了原缺的三份);但**全部测试用例数一个未复现**、
   40 余份 `*-raw-*` 分段底稿未读、R1 附卷 170 条能力点只抽了 2 条、Mermaid 未实渲染,
   且**原定的对抗复验环节被整段砍掉**——本报告全部结论都只经过评审位一人取证。
