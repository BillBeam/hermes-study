...
5. 最值得抄走的一条不是某个机制,而是一个**对照实验**:配置键的文档覆盖率约 87.7%,
   环境变量是 **100%**。差别不在纪律,在**说明文字是不是数据结构里的必填字段**。
...
| | 数量 | 零文档 | 覆盖率 |
|---|---|---|---|
| 配置键(`DEFAULT_CONFIG`) | 856 | 105 | 约 87.7%(且这是**上限**) |
| 环境变量(`OPTIONAL_ENV_VARS`) | 151 | **0** | **100%** |

差别不是有人更勤快。配置键的说明是**自由文本注释**,而正式文档写在另一个目录的
另一个文件里,两处各自演化;环境变量的说明是**定义字面量里的必填字段**
(`description` / `prompt` / `url` / `password` / `category`),定义与说明**同一处、同一次编辑**。
**覆盖率于是不再依赖纪律,而依赖类型。** 你要漏,得先删一个字段。
