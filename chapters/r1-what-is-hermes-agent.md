...
- **规模**:全仓 8,530 个文件、260 万行文本。Python 是主体(核心逻辑),TypeScript 是界面层,
  剩下大量是技能文档、翻译、测试。真正的"harness 核心机制"约 38 万行(见下面的分层)。
...
学一个 26 万行文本的仓库,最怕"黑洞"——某些文件从来没人交代过。所以这个学习项目的第一件事,是把
**每一个文件**都归进一个"学习层",各层行数加总必须等于全仓总行数,并用脚本校验。这样"学完"才有一个
可检验的定义:所有文件都不再是"未处理"状态。

五个层(`data/ledger.tsv` 台账,`scripts/verify_ledger.py` 校验):
...
一个反直觉的数字:**测试(756,619 行)比核心机制(382,770 行)还多**。这不是坏事——它意味着几乎每个
...
执行(`agent/agent_init.py:892 @ 863e313` 只初始化为 False,`agent/conversation_loop.py:1445` 只消费为
