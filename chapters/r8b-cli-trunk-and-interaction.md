...
4. **本轮最值得记的不是某个 bug,而是一个反复出现的形状**:一次重命名 / 一次修复,**传到了一部分消费面、没传到另一部分**,而落后的那一面**不会报错**。本章拿到了这个形状的**三个独立实例**,其中一个是"启动时的工具告警面板从来没显示过"。
...
- **▲-R8B-01** 两份 TUI 判定对 `HERMES_TUI=1` 的语义分类相反(explicit vs ambient),**当前被三处独立防护完全补偿,不记缺陷**。详见 §4.3。
