...
- **WAL 启用要信"返回值"而不是"没抛异常"**。`PRAGMA journal_mode=WAL` 在 macOS NFS/SMB 上有个恶劣
  ...
  (hermes_state.py:743-767)。识别"WAL 不兼容"用一张错误指纹表(`locking protocol` / `not authorized`
...
- **零化库隔离而不删除**。某类崩溃会留下一个"大小非零但全是 NUL 字节"的 state.db。检测到后不删,而是
  ...
  进程对同一个活文件重复动手(hermes_state.py:1815-1827,#68805 复盘)。
...
- **打开期多级自修复**。上面那个"有文件却显示没有会话"的场景,根因通常是 `sqlite_master`(SQLite 记录
  ...
  复验,修好即止(hermes_state.py:1370-1535)。关键洞察:**canonical 的消息数据一直是好的,坏的只是
...
会在一个兄弟进程正常持锁几秒(比如 VACUUM)时静默丢掉用户一轮,还误导运维去查磁盘(hermes_state.py:1908-1947)。
...
`ALTER TABLE ADD` 缺的列(hermes_state_schema.py:292-348)。加一列 = 只改 `SCHEMA_SQL` 一处,下次启动
...
(context_compressor.py:1318-1327):① 无 LLM 地剪掉旧工具结果 → ② 保护头部(系统提示 + 第一轮)→
...
- **触发要"双重测量 + 防抖"**,否则会抖成冻屏。模型每轮报两个 token 数:请求前的粗估(对 schema 重的
  ...
  裁一次——因为粗估会每轮上下跳,放在那里判会把计数器每轮清零、反复重开压缩循环(context_compressor.py:2506-2512)。
...
(context_compressor.py:193-211)。修法:按"模板数得到的可见角色"选角,而不是字面顺序;双撞时把摘要
并入尾部消息。
...
**工具面四形态**(session_search_tool.py:863-873):**discovery**(给 query,宽扫后按会话世系去重,每个
...
复用前缀内,变了只重刷尾部(system_prompt.py:503-513)。**时间戳只到日期精度**("Conversation started:
2026 年 8 月 7 日"),让系统提示全天字节稳定;模型真要精确时间可以用工具查(system_prompt.py:537-543)。

**项目上下文文件按优先级选一个,不合并**(prompt_builder.py:2188-2196):`.hermes.md → AGENTS.md →
CLAUDE.md → .cursorrules`,首中即停。超长文件按模型窗口的 6% 动态截断(保 70% 头 + 20% 尾,中缝留一句
"用 read_file 读全文"),并向用户状态信道发告警。注入前**全部过威胁扫描**——命中就把整个文件替换成
`[BLOCKED]` 占位符,因为"这个文件会原样进系统提示,用户没机会中途拦截"(prompt_builder.py:66-79)。
...
- **内建记忆是两个纯文本文件**(`~/.hermes/memories/MEMORY.md`、`USER.md`),会话开始时**冻结成快照**进
  ...
  快照(memory_tool.py:11-14)。工具响应展示实时状态弥补这个延迟。

- **外部记忆后端当"可以宕的慢速旁路"**。读路径(召回)每个 provider 一个守护线程 + 8 秒超时,卡死就
  跳过、后续回合零成本绕过,直到那个卡住的调用自己返回(memory_manager.py:547-595)。写路径走一个惰性
  单 worker 后台线程,完全离线——因为一个配错的记忆守护进程被实测阻塞过约 298 秒,内联做会让整个界面
  显示 agent "运行中"好几分钟。

**三道防注入围栏**(memory_manager.py:347-361):① 召回内容包进 `<memory-context>` 标签 + 一句"这是
...
式 CLI 前台弹审批,网关/脚本/后台一律 staged 落盘待审;门禁**只延迟、从不静默丢弃**(memory_tool.py:941-947)。
...
存快照,LLM 完全看不见它,是透明基础设施(checkpoint_manager.py:1-12)。它和 state.db 是两套独立系统,
