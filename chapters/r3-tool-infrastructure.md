...
flowchart TD
    ...
    C2 -->|否| C2b{sudo stdin 守卫?<br/>没配密码却往 sudo -S 灌}
    C2b -->|命中| BLOCK1b[无条件拦截<br/>第二道地板]
    C2b -->|否| C3{用户自定义的deny规则?}
    C3 -->|命中| BLOCK2[拦截]
    C3 -->|否| C4{免确认模式 / 审批关闭?}
    C4 -->|是| PASS2[直通]
    C4 -->|否| C5{永久放行清单?}
...
关键在顺序:**hardline(硬底线)和用户 deny 规则,排在"免确认模式"之前**(`tools/approval.py:3761 @ 863e313` 早于
...
**地板其实有两道,不是一道。** hardline 和用户 deny 之间还夹着第三道无条件拦截——
**sudo stdin 守卫**:agent 在**没有配置 `SUDO_PASSWORD`** 的情况下试图往 `sudo -S` 灌东西,
一律拦下。它的代码注释自己就点明了这层关系:

`tools/approval.py:3767 @ 863e313`

```python
    # Like the hardline floor above, this is unconditional: there is never a
```

`tools/approval.py:3769 @ 863e313`

```python
    # SUDO_PASSWORD has been configured.  This must fire BEFORE the yolo
```

**为什么它没被并进 hardline,这本身是个值得讲的取舍。** hardline 那张表刻意保持很小
(测试断言不超过 20 条),它收的是"**这条命令本身没有恢复路径**"——删根、格式化、关机。
而 sudo stdin 守卫拦的不是某条命令,是一个**上下文条件**:同样一句 `sudo -S ...`,
配了 `SUDO_PASSWORD` 就是正常用法,没配就是 agent 在猜密码。
**一张纯模式匹配的表装不下"要看配置状态"的判断**,所以它单列一道。
换句话说:hardline 按**命令的性质**拦,sudo 守卫按**当前环境是否授权过这件事**拦;
两者都在开关之前,但判据不同类——这正是"地板不能只有一张表"的理由
(review-1 建议-8 / M-12)。
...
- **第一层(URL 预检)**:抓取前解析域名、检查每个 IP。有三条"永远拦"的地板,排在任何"允许私网"开关
  ...
  (`tools/url_safety.py:437 @ 863e313` 起)。这些"从来不是 agent 合法要访问的目标"。
...
- **环境洗净**:子进程如果继承父进程的完整环境变量,`os.environ` 里的 `OPENAI_API_KEY` 就泄了。所以派生
  ...
  (`tools/code_execution_tool.py:253 @ 863e313`)。这里有个真实的攻击面被堵过:一个恶意技能想在配置里声明"请把
...
- **审批跨线程传播 + 输出脱敏**:脚本里的 `terminal("rm -rf /")` 该不该审批?RPC 派发发生在父进程的一个
  ...
  (`tools/code_execution_tool.py:1638 @ 863e313`)。
...
不能借桥接摸到整个进程的工具(`tools/tool_search.py` + `model_tools.py:1213 @ 863e313`)。
